...
<div class="wrapper bgded" style="background-color:#ffffff">
  <div id="pageintro" class="hoc clear"> 
   ...
   echo '<p class="centre"  style="color:black">Vous  voulez faire une recherche ? <a href="rechercherhemploi1.php">Rechercher</a></p></div>';
   $requete= $bdd->prepare("SELECT * FROM rh_emploi WHERE date LIKE ? ORDER BY id DESC");
                         ...
                         echo "<tr>";
                      echo "<td class='table warning'>".$row[$i]["code_fct"]."</td>";   
                           echo "<td class='table warning'>".$row[$i]["code_act"]."</td>";    
                           echo "<td class='table warning'>".$row[$i]["nom_prenom"]."</td>";           
                         echo "<td class='table warning'>".$row[$i]["historique"]."</td>";   

                         if ($row[$i]["remarque"]!=="") {
                           echo "<td class='table warning' ><a href='modifierRqrhemploi1.php?remarque=".$row[$i]["remarque"]."&code_fct=".$row[$i]["code_fct"]."&code_act=".$row[$i]["code_act"]."'>".$row[$i]["remarque"]."</a></td>";   
                          } 
                         else{
                          echo "<td class='table warning' ><a href='modifierRqrhemploi1.php?remarque=".$row[$i]["remarque"]."&code_fct=".$row[$i]["code_fct"]."&code_act=".$row[$i]["code_act"]."'>Editer</a></td>";
                         }
                             echo "<td class='table warning'><a href='rhemploicomplet1.php?code_fct=".$row[$i]["code_fct"]."&code_act=".$row[$i]["code_act"]."'>details</a></td>";   
                        echo "</tr>";
