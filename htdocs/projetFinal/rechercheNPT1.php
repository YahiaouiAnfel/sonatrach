...
$historique=$_POST['date'];
  $requete= $bdd->prepare("SELECT * FROM npt_exp WHERE historique LIKE ?  ORDER BY id DESC");
                      ...
                      echo "<form method='get'>";
                         ...
                         if ($row[$i]["remarque"]!=="") {
                           echo "<td class='table warning' ><a href='modifierRqNPT1.php?remarque=".$row[$i]["remarque"]."&dev=".$row[$i]["Temps_Productif"]."&historique=".$row[$i]["historique"]."'>".$row[$i]["remarque"]."</a></td>";   
                          } 
                         else{
                         echo "<td class='table warning' ><a href='modifierRqNPT1.php?remarque=".$row[$i]["remarque"]."&dev=".$row[$i]["Temps_Productif"]."&historique=".$row[$i]["historique"]."'>Editer</a></td>"; 
                         }
                             echo "<td class='table warning'><a href='NPTcomplet1.php?dev=".$row[$i]["Temps_Productif"]."&historique=".$row[$i]["historique"]."'>details</a></td>";         
                          echo "</tr>";
                            $i++;}
                      echo "</form>";
...
$historique=$_POST['date'];
  $requete= $bdd->prepare("SELECT * FROM npt_dev WHERE historique LIKE ?  ORDER BY id DESC");
                          $requete->execute(array('%'.$historique.'%'));
                          $existe= $requete->rowCount();
                          $row = $requete->fetchAll(PDO::FETCH_ASSOC);
                          $i=0;
  ...
  $requete= $bdd->prepare("SELECT * FROM npt_exp ORDER BY id DESC");
 ...
 if ($_GET['var']==='dev') {
  ...
  $requete= $bdd->prepare("SELECT * FROM npt_dev  ORDER BY id DESC");
                          $requete->execute(array());
                          $existe= $requete->rowCount();
                          $row = $requete->fetchAll(PDO::FETCH_ASSOC);
                          $i=0;
