...
<div class="wrapper bgded" style="background-color:#ffffff">
  <div id="pageintro" class="hoc clear"> 
   ...
   echo '<p class="centre" style="color:black">Vous  voulez faire une recherche ? <a href="recherchejur1.php">Rechercher</a></p></div>';
   $requete= $bdd->prepare("SELECT * FROM juridique WHERE date LIKE ? ORDER BY id DESC");
                      ...
                      echo "<td class='table warning'>".$row[$i]["num_Contrat_Avenant_Commande"]."</td>";   
                         ...
                         if ($row[$i]["remarque"]!=="") {
                           echo "<td class='table warning' ><a href='modifierRqJuridique1.php?remarque=".$row[$i]["remarque"]."&num_Contrat_Avenant_Commande=".$row[$i]["num_Contrat_Avenant_Commande"]."&numero_de_contrat_de_base=".$row[$i]["numero_de_contrat_de_base"]."'>".$row[$i]["remarque"]."</a></td>";   
                          } 
                         else{
                          echo "<td class='table warning' ><a href='modifierRqJuridique1.php?remarque=".$row[$i]["remarque"]."&num_Contrat_Avenant_Commande=".$row[$i]["num_Contrat_Avenant_Commande"]."&numero_de_contrat_de_base=".$row[$i]["numero_de_contrat_de_base"]."'>Editer</a></td>";
                         }
                             echo "<td class='table warning'><a href='juridiquecomplet1.php?num_Contrat_Avenant_Commande=".$row[$i]["num_Contrat_Avenant_Commande"]."&numero_de_contrat_de_base=".$row[$i]["numero_de_contrat_de_base"]."'>details</a></td>";   
                        echo "</tr>";
