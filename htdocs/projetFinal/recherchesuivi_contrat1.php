...
$historique=$_POST['date'];
  $requete= $bdd->prepare("SELECT * FROM contrat WHERE historique LIKE ?  ORDER BY id DESC");
                     ...
                     echo "<th class='table warning '>Numéro contrat</th>";    
                      ...
                      echo "<form method='get'>";
                         ...
                         if ($row[$i]["remarque"]!=="") {
                           echo "<td class='table warning' ><a href='modifierRqsuivi_contrat1.php?remarque=".$row[$i]["remarque"]."&Num_contrat=".$row[$i]["Num_contrat"]."&historique=".$row[$i]["historique"]."'>".$row[$i]["remarque"]."</a></td>";   
                          } 
                         else{
                         echo "<td class='table warning' ><a href='modifierRqsuivi_contrat1.php?remarque=".$row[$i]["remarque"]."&Num_contrat=".$row[$i]["Num_contrat"]."&historique=".$row[$i]["historique"]."'>Editer</a></td>"; 
                         }
                             echo "<td class='table warning'><a href='suivi_contratcomplet1.php?var1=".$row[$i]["Num_contrat"]."&historique=".$row[$i]["historique"]."'>details</a></td>";         
                          echo "</tr>";
                            $i++;}
                      echo "</form>";
...
/* if ($_GET['var']==='appel_offre') {
    echo "<div class='ferme' align='center'>";


$historique=$_POST['date'];
  $requete= $bdd->prepare("SELECT * FROM appel_offres WHERE historique LIKE ?  ORDER BY id DESC");
                          $requete->execute(array('%'.$historique.'%'));
                          $existe= $requete->rowCount();
                          $row = $requete->fetchAll(PDO::FETCH_ASSOC);
                          $i=0;
                      ...
                      echo "<form method='get'>";
                         ...
                         if ($row[$i]["remarque"]!=="") {
                           echo "<td class='table warning' ><a href='modifierRqsuivi_contrat.php?remarque=".$row[$i]["remarque"]."&Numero_Appel_Offres=".$row[$i]["Numero_Appel_Offres"]."&Num_Visa_definitive=".$row[$i]["Num_Visa_definitive"]."'>".$row[$i]["remarque"]."</a></td>";   
                          } 
                         else{
                         echo "<td class='table warning' ><a href='modifierRqsuivi_contrat.php?remarque=".$row[$i]["remarque"]."&Numero_Appel_Offres=".$row[$i]["Numero_Appel_Offres"]."&Num_Visa_definitive=".$row[$i]["Num_Visa_definitive"]."'>Editer</a></td>"; 
                         }
                             echo "<td class='table warning'><a href='suivi_contratcomplet.php?var2=".$row[$i]["Numero_Appel_Offres"]."&Num_Visa_definitive=".$row[$i]["Num_Visa_definitive"]."'>details</a></td>";         
                          echo "</tr>";
                            $i++;}
                      echo "</form>";
  ...
  $requete= $bdd->prepare("SELECT * FROM contrat ORDER BY id DESC");
...
/* if ($_GET['var']==='appel_offre') {
  ...
  $requete= $bdd->prepare("SELECT * FROM appel_offres  ORDER BY id DESC");
                          $requete->execute(array());
                          $existe= $requete->rowCount();
                          $row = $requete->fetchAll(PDO::FETCH_ASSOC);
                          $i=0;
