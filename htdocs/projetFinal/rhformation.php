...
<br>
   ...
   echo '<p class="centre"  style="color:black">Vous  voulez faire une recherche ? <a href="rechercherhformation.php">Rechercher</a></p></div>';
   $requete= $bdd->prepare("SELECT * FROM rh_formation WHERE date LIKE ? ORDER BY id DESC");
                          ...
                          $i=0;
                           ...
                           echo "<th class='table warning '>Code formation</th>"; 
                            ...
                            echo "<th class='table warning'>Remarque</th>";  
                               ...
                               echo "<th class='table warning '><a href='indication.php'>Indication</a></th>"; 
                      ...
                      echo "<td class='table warning'>".$row[$i]["matricule"]."</td>";   
                           echo "<td class='table warning'>".$row[$i]["nom_prenom"]."</td>";    
                           echo "<td class='table warning'>".$row[$i]["code_formation"]."</td>";           
                         echo "<td class='table warning'>".$row[$i]["historique"]."</td>";   

                         if ($row[$i]["remarque"]!=="") {
                           echo "<td class='table warning' ><a href='modifierRqrhformation.php?remarque=".$row[$i]["remarque"]."&matricule=".$row[$i]["matricule"]."&code_formation=".$row[$i]["code_formation"]."'>".$row[$i]["remarque"]."</a></td>";   
                          } 
                         else{
                          echo "<td class='table warning' ><a href='modifierRqrhformation.php?remarque=".$row[$i]["remarque"]."&matricule=".$row[$i]["matricule"]."&code_formation=".$row[$i]["code_formation"]."'>Editer</a></td>";
                         }
                             echo "<td class='table warning'><a href='rhformationcomplet.php?matricule=".$row[$i]["matricule"]."&code_formation=".$row[$i]["code_formation"]."'>details</a></td>";   
                        echo "</tr>";
