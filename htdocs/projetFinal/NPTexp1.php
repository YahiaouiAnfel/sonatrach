...
<div class="wrapper bgded" style="background-color:#ffffff">
  ...
  <h2 align="center" style="color: black"> NPT EXP</h2><hr>
<?php
$date = new DateTime();
echo "<div  class='col-sm-12'>";
   echo "<div class='ferme' align='center'>";
  echo "<div class='ferme' align='center'>";
  echo '<p class="centre" style="color:black">Vous  voulez faire une recherche ? <a href="rechercheNPT1.php?var=exp">Rechercher</a></p>';
      ...
      echo "<table border='1' class=' table table-bordered table-striped table-responsive table-hover table-condensed' style='color:black'>";    
                      ...
                      echo "<form method='get'>";
                         ...
                         if ($row[$i]["remarque"]!=="") {
                           echo "<td class='table warning' ><a href='modifierRqNPT1.php?remarque=".$row[$i]["remarque"]."&exp=".$row[$i]["Temps_Productif"]."&historique=".$row[$i]["historique"]."'>".$row[$i]["remarque"]."</a></td>";   
                          } 
                         else{
                         echo "<td class='table warning' ><a href='modifierRqNPT1.php?remarque=".$row[$i]["remarque"]."&exp=".$row[$i]["Temps_Productif"]."&historique=".$row[$i]["historique"]."'>Editer</a></td>"; 
                         }
                             echo "<td class='table warning'><a href='NPTcomplet1.php?exp=".$row[$i]["Temps_Productif"]."&historique=".$row[$i]["historique"]."'>details</a></td>";         
                          echo "</tr>";
                            $i++;}
                      echo "</form>";
